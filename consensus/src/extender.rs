...
impl<H: Hasher> Extender<H> {
    ...
    fn add_unit(&mut self, u: ExtenderUnit<H>) {
        debug!(target: "Stance-extender", "{:?} New unit in Extender round {:?} creator {:?} hash {:?}.", self.node_id, u.round, u.creator, u.hash);
        let round = u.round;
    ...
    fn finalize_round(&mut self, round: Round, head: &H::Hash) {
        ...
        if self.finalizer_tx.unbounded_send(batch).is_err() {
            warn!(target: "Stance-extender", "{:?} Channel for batches should be open", self.node_id);
            self.exiting = true;
        }

        debug!(target: "Stance-extender", "{:?} Finalized round {:?} with head {:?}.", self.node_id, round, head);
        ...
        loop {
            futures::select! {
                ...
                _ = &mut terminator.get_exit() => {
                    debug!(target: "Stance-extender", "{:?} received exit signal.", self.node_id);
                    self.exiting = true;
                }
            }
            if self.exiting {
                debug!(target: "Stance-extender", "{:?} Extender decided to exit.", self.node_id);
...
mod tests {
    ...
    async fn finalize_rounds_01() {
        ...
        let extender_handle = tokio::spawn(async move {
            extender
                .extend(Terminator::create_root(exit_rx, "Stance-extender"))
                .await
        });
