    ...
    S: Signature,
{
    ...
    fn on_request_coord(&mut self, coord: UnitCoord) {
        trace!(target: "Stance-member", "{:?} Dealing with missing coord notification {:?}.", self.index(), coord);
    ...
    fn send_unit_message(&mut self, message: UnitMessage<H, D, S>, recipient: Recipient) {
        if self
            ...
            .is_err()
        {
            warn!(target: "Stance-member", "{:?} Channel to network should be open", self.index());
            self.exiting = true;
        }
    }
    ...
    fn status_report(&self) {
        ...
        info!(target: "Stance-member", "{}", status);
    }
        ...
        loop {
            futures::select! {
                event = self.notifications_from_runway.next() => match event {
                    ...
                    None => {
                        error!(target: "Stance-member", "{:?} Unit message stream from Runway closed.", self.index());
                        break;
                    },
                },
                ...
                event = self.resolved_requests.next() => match event {
                    ...
                    None => {
                        error!(target: "Stance-member", "{:?} Resolved-requests stream from Runway closed.", self.index());
                        break;
                    }
                },
                ...
                event = self.unit_messages_from_network.next() => match event {
                    Some(message) => match message.try_into() {
                        ...
                        Err(_) => error!(target: "Stance-member", "{:?} Unable to convert a UnitMessage into an instance of RunwayNotificationIn.", self.index()),
                    },
                    None => {
                        error!(target: "Stance-member", "{:?} Unit message stream from network closed.", self.index());
                        break;
                    },
                },
                ...
                _ = &mut terminator.get_exit() => {
                    debug!(target: "Stance-member", "{:?} received exit signal", self.index());
                    self.exiting = true;
                },
            }
            if self.exiting {
                debug!(target: "Stance-member", "{:?} Member decided to exit.", self.index());
                terminator.terminate_sync().await;
                break;
            }
        }

        debug!(target: "Stance-member", "{:?} Member stopped.", self.index());
    }
    ...
    fn send_notification_to_runway(&mut self, notification: RunwayNotificationIn<H, D, S>) {
        if self
            ...
            .is_err()
        {
            warn!(target: "Stance-member", "{:?} Sender to runway with RunwayNotificationIn messages should be open", self.index());
            self.exiting = true;
        }
    }
}
...
>(
    ...
    info!(target: "Stance-member", "{:?} Starting a new session.", index);
    debug!(target: "Stance-member", "{:?} Spawning party for a session.", index);
    ...
    debug!(target: "Stance-member", "{:?} Spawning network.", index);
    let network_terminator = terminator.add_offspring_connection("Stance-network");
    ...
    debug!(target: "Stance-member", "{:?} Network spawned.", index);

    debug!(target: "Stance-member", "{:?} Initializing Runway.", index);
    let runway_terminator = terminator.add_offspring_connection("Stance-runway");
    ...
    debug!(target: "Stance-member", "{:?} Runway spawned.", index);

    debug!(target: "Stance-member", "{:?} Initializing Member.", index);
    ...
    debug!(target: "Stance-member", "{:?} Member initialized.", index);

    futures::select! {
        _ = network_handle => {
            error!(target: "Stance-member", "{:?} Network-hub terminated early.", index);
        },

        _ = runway_handle => {
            error!(target: "Stance-member", "{:?} Runway terminated early.", index);
        },

        _ = member_handle => {
            error!(target: "Stance-member", "{:?} Member terminated early.", index);
        },

        _ = &mut terminator.get_exit() => {
            debug!(target: "Stance-member", "{:?} exit channel was called.", index);
        },
    }

    debug!(target: "Stance-member", "{:?} Run ending.", index);

    terminator.terminate_sync().await;

    handle_task_termination(network_handle, "Stance-member", "Network", index).await;
    handle_task_termination(runway_handle, "Stance-member", "Runway", index).await;
    handle_task_termination(member_handle, "Stance-member", "Member", index).await;

    info!(target: "Stance-member", "{:?} Session ended.", index);
}
