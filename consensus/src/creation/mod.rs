    ...
    loop {
        ...
        futures::select! {
            unit = incoming_parents.next() => match unit {
                ...
                None => {
                    debug!(target: "Stance-creator", "Incoming parent channel closed, exiting.");
                    return Err(());
                }
            },
            _ = &mut delay => {
                if can_create {
                    warn!(target: "Stance-creator", "Delay passed despite us not waiting for it.");
                }
                can_create = true;
                delay = Delay::new(Duration::from_secs(30 * 60)).fuse();
            },
            _ = exit => {
                debug!(target: "Stance-creator", "Received exit signal.");
                return Err(());
            },
        }
    }
...
pub async fn run<H: Hasher>(
    ...
    let starting_round = futures::select! {
        maybe_round =  starting_round => match maybe_round {
            Ok(Some(round)) => round,
            Ok(None) => {
                warn!(target: "Stance-creator", "None starting round provided. Exiting.");
                return;
            }
            Err(e) => {
                error!(target: "Stance-creator", "Starting round not provided: {}", e);
                return;
            }
        },
    ...
    debug!(target: "Stance-creator", "Creator starting from round {}", starting_round);
    for round in starting_round..max_round {
        ...
        {
            warn!(target: "Stance-creator", "Notification send error: {}. Exiting.", e);
            return;
        }
    }

    warn!(target: "Stance-creator", "Maximum round reached. Not creating another unit.");
}
