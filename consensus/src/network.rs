    ...
    > NetworkHub<H, D, S, MS, N>
{
        ...
        match network_data {
            Units(unit_message) => {
                if let Err(e) = self.units_received.unbounded_send(unit_message) {
                    warn!(target: "Stance-network-hub", "Error when sending units to consensus {:?}", e);
                }
            }

            Alert(alert_message) => {
                if let Err(e) = self.alerts_received.unbounded_send(alert_message) {
                    warn!(target: "Stance-network-hub", "Error when sending alerts to consensus {:?}", e);
                }
            }
        }
    }
    ...
    async fn run(mut self, mut terminator: Terminator) {
        loop {
            ...
            futures::select! {
                unit_message = self.units_to_send.next() => match unit_message {
                    ...
                    None => {
                        error!(target: "Stance-network-hub", "Outgoing units stream closed.");
                        break;
                    }
                },
                alert_message = self.alerts_to_send.next() => match alert_message {
                    ...
                    None => {
                        error!(target: "Stance-network-hub", "Outgoing alerts stream closed.");
                        break;
                    }
                },
                incoming_message = self.network.next_event().fuse() => match incoming_message {
                    ...
                    None => {
                        error!(target: "Stance-network-hub", "Network stopped working.");
                        break;
                    }
                },
        ...
        debug!(target: "Stance-network-hub", "Network ended.");
    }
}
