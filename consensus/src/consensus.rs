...
pub(crate) async fn run<H: Hasher + 'static>(
    ...
    debug!(target: "Stance", "{:?} Starting all services...", conf.node_ix);
    ...
    let extender_terminator = terminator.add_offspring_connection("Stance-extender");
    ...
    let mut terminal_handle = spawn_handle
        ...
        .fuse();
    debug!(target: "Stance", "{:?} All services started.", index);

    futures::select! {
        _ = terminator.get_exit() => {},
        _ = terminal_handle => {
            debug!(target: "Stance-consensus", "{:?} terminal task terminated early.", index);
        },
        _ = creator_handle => {
            debug!(target: "Stance-consensus", "{:?} creator task terminated early.", index);
        },
        _ = extender_handle => {
            debug!(target: "Stance-consensus", "{:?} extender task terminated early.", index);
        }
    }
    debug!(target: "Stance", "{:?} All services stopping.", index);

    // we stop no matter if received Ok or Err
    terminator.terminate_sync().await;

    handle_task_termination(terminal_handle, "Stance-consensus", "Terminal", index).await;
    handle_task_termination(creator_handle, "Stance-consensus", "Creator", index).await;
    handle_task_termination(extender_handle, "Stance-consensus", "Extender", index).await;

    debug!(target: "Stance", "{:?} All services stopped.", index);
}
