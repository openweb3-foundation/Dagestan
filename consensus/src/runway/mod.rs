    ...
    MK: MultiKeychain,
{
    ...
    async fn on_unit_message(&mut self, message: RunwayNotificationIn<H, D, MK::Signature>) {
        match message {
            RunwayNotificationIn::NewUnit(u) => {
                trace!(target: "Stance-runway", "{:?} New unit received {:?}.", self.index(), &u);
                self.on_unit_received(u, false)
            }

            RunwayNotificationIn::Request(request, node_id) => match request {
                Request::Coord(coord) => {
                    trace!(target: "Stance-runway", "{:?} Coords request received {:?}.", self.index(), coord);
                    self.on_request_coord(node_id, coord)
                }
                Request::Parents(u_hash) => {
                    trace!(target: "Stance-runway", "{:?} Parents request received {:?}.", self.index(), u_hash);
                    self.on_request_parents(node_id, u_hash)
                }
                Request::NewestUnit(salt) => {
                    trace!(target: "Stance-runway", "{:?} Newest unit request received {:?}.", self.index(), salt);
            ...
            RunwayNotificationIn::Response(res) => match res {
                Response::Coord(u) => {
                    trace!(target: "Stance-runway", "{:?} Fetch response received {:?}.", self.index(), &u);
                    self.on_unit_received(u, false)
                }
                Response::Parents(u_hash, parents) => {
                    trace!(target: "Stance-runway", "{:?} Response parents received {:?}.", self.index(), u_hash);
                    self.on_parents_response(u_hash, parents)
                }
                Response::NewestUnit(response) => {
                    trace!(target: "Stance-runway", "{:?} Response newest unit received from {:?}.", self.index(), response.index());
                    let res = self.responses_for_collection.unbounded_send(response);
                    if res.is_err() {
                        debug!(target: "Stance-runway", "{:?} Could not send response to collection ({:?}).", self.index(), res)
                    }
                }
            },
        }
    ...
    fn on_unit_received(&mut self, uu: UncheckedSignedUnit<H, D, MK::Signature>, alert: bool) {
        match self.validator.validate_unit(uu) {
            ...
            Err(e) => warn!(target: "Stance-member", "Received unit failing validation: {}", e),
        }
    }
    ...
    fn add_unit_to_store_unless_fork(&mut self, su: SignedUnit<H, D, MK>) {
        ...
        trace!(target: "Stance-member", "{:?} Adding member unit to store {:?}", self.index(), full_unit);
        if self.store.is_forker(full_unit.creator()) {
            trace!(target: "Stance-member", "{:?} Ignoring forker's unit {:?}", self.index(), full_unit);
            return;
        }
    ...
    fn on_new_forker_detected(&mut self, forker: NodeIndex, proof: ForkProof<H, D, MK::Signature>) {
        ...
        if self.alerts_for_alerter.unbounded_send(alert).is_err() {
            warn!(target: "Stance-runway", "{:?} Channel to alerter should be open", self.index());
            self.exiting = true;
        }
    }
    ...
    fn on_request_coord(&mut self, node_id: NodeIndex, coord: UnitCoord) {
        debug!(target: "Stance-runway", "{:?} Received fetch request for coord {:?} from {:?}.", self.index(), coord, node_id);
        let maybe_su = (self.store.unit_by_coord(coord)).cloned();

        if let Some(su) = maybe_su {
            trace!(target: "Stance-runway", "{:?} Answering fetch request for coord {:?} from {:?}.", self.index(), coord, node_id);
            self.send_message_for_network(RunwayNotificationOut::Response(
                Response::Coord(su.into()),
                node_id,
            ));
        } else {
            trace!(target: "Stance-runway", "{:?} Not answering fetch request for coord {:?}. Unit not in store.", self.index(), coord);
        }
    }

    fn on_request_parents(&mut self, node_id: NodeIndex, u_hash: H::Hash) {
        debug!(target: "Stance-runway", "{:?} Received parents request for hash {:?} from {:?}.", self.index(), u_hash, node_id);

        if let Some(p_hashes) = self.store.get_parents(u_hash) {
            let p_hashes = p_hashes.clone();
            trace!(target: "Stance-runway", "{:?} Answering parents request for hash {:?} from {:?}.", self.index(), u_hash, node_id);
            ...
            for hash in p_hashes.iter() {
                if let Some(fu) = self.store.unit_by_hash(hash) {
                    ...
                    debug!(target: "Stance-runway", "{:?} Not answering parents request, one of the parents missing from store.", self.index());
            ...
            trace!(target: "Stance-runway", "{:?} Not answering parents request for hash {:?}. Unit not in DAG yet.", self.index(), u_hash);
        }
    }
    ...
    async fn on_request_newest(&mut self, requester: NodeIndex, salt: u64) {
        ...
        {
            error!(target: "Stance-runway", "Unable to send response to network: {}", e);
        }
    }
    ...
    fn on_parents_response(
        ...
        if self.store.get_parents(u_hash).is_some() {
            trace!(target: "Stance-runway", "{:?} We got parents response but already know the parents.", self.index());
            return;
        }
        let (u_round, u_control_hash, parent_ids) = match self.store.unit_by_hash(&u_hash) {
            ...
            None => {
                trace!(target: "Stance-runway", "{:?} We got parents but don't even know the unit. Ignoring.", self.index());
                return;
            }
        };

        if parent_ids.len() != parents.len() {
            warn!(target: "Stance-runway", "{:?} In received parent response expected {} parents got {} for unit {:?}.", self.index(), parents.len(), parent_ids.len(), u_hash);
            return;
        }
        ...
        for (i, uu) in parents.into_iter().enumerate() {
            let su = match self.validator.validate_unit(uu) {
                Ok(su) => su,
                Err(e) => {
                    warn!(target: "Stance-runway", "{:?} In received parent response received a unit that does not pass validation: {}", self.index(), e);
                    return;
                }
            };
            let full_unit = su.as_signable();
            if full_unit.round() + 1 != u_round {
                warn!(target: "Stance-runway", "{:?} In received parent response received a unit with wrong round.", self.index());
                return;
            }
            if full_unit.creator() != parent_ids[i] {
                warn!(target: "Stance-runway", "{:?} In received parent response received a unit with wrong creator.", self.index());
                return;
            }
        ...
        if ControlHash::<H>::combine_hashes(&p_hashes_node_map) != u_control_hash {
            warn!(target: "Stance-runway", "{:?} In received parent response the control hash is incorrect {:?}.", self.index(), p_hashes_node_map);
            return;
        }
        let p_hashes: Vec<_> = p_hashes_node_map.into_values().collect();
        self.store.add_parents(u_hash, p_hashes.clone());
        trace!(target: "Stance-runway", "{:?} Succesful parents response for {:?}.", self.index(), u_hash);
    ...
    fn save_unit(&mut self, uu: UncheckedSignedUnit<H, D, MK::Signature>) {
        ...
        trace!(target: "Stance-runway", "{:?} Saving a created unit {:?}.", self.index(), h);
        if let Err(err) = self.unit_saver.save(uu) {
            error!(target: "Stance-runway", "{:?} Failed to save unit {:?}. {}", self.index(), h, err);
        } else {
            trace!(target: "Stance-runway", "{:?} Saved a created unit {:?}.", self.index(), h);
        }
    }

    fn on_packed(&mut self, signed_unit: SignedUnit<H, D, MK>) {
        debug!(target: "Stance-runway", "{:?} On create notification.", self.index());
        ...
        match notification {
            NotificationOut::CreatedPreUnit(pu, _) => {
                if self.preunits_for_packer.unbounded_send(pu).is_err() {
                    warn!(target: "Stance-runway", "{:?} preunits_for_packer channel should be open", self.index());
                    self.exiting = true;
                }
            }
            ...
            NotificationOut::AddedToDag(h, p_hashes) => {
                ...
                if let Some(su) = self.store.unit_by_hash(&h).cloned() {
                    ...
                    if su.as_signable().creator() == self.index() {
                        trace!(target: "Stance-runway", "{:?} Sending a unit {:?}.", self.index(), h);
                        self.send_message_for_network(RunwayNotificationOut::NewSelfUnit(
                            su.into(),
                        ));
                    }
                } else {
                    error!(target: "Stance-runway", "{:?} A unit already added to DAG is not in our store: {:?}.", self.index(), h);
                }
            }
        }
    }
    ...
    fn on_missing_coords(&mut self, mut coords: Vec<UnitCoord>) {
        trace!(target: "Stance-runway", "{:?} Dealing with missing coords notification {:?}.", self.index(), coords);
    ...
    fn on_wrong_control_hash(&mut self, u_hash: H::Hash) {
        trace!(target: "Stance-runway", "{:?} Dealing with wrong control hash notification {:?}.", self.index(), u_hash);
        if let Some(p_hashes) = self.store.get_parents(u_hash) {
            // We have the parents by some strange reason (someone sent us parents
            // without us requesting them).
            let p_hashes = p_hashes.clone();
            trace!(target: "Stance-runway", "{:?} We have the parents for {:?} even though we did not request them.", self.index(), u_hash);
    ...
    fn send_message_for_network(
        ...
        if self
            ...
            .is_err()
        {
            warn!(target: "Stance-runway", "{:?} unit_messages_for_network channel should be open", self.index());
            self.exiting = true;
        }
    }
    ...
    fn send_resolved_request_notification(&mut self, notification: Request<H>) {
        if self.resolved_requests.unbounded_send(notification).is_err() {
            warn!(target: "Stance-runway", "{:?} resolved_requests channel should be open", self.index());
            self.exiting = true;
        }
    }
    ...
    fn send_consensus_notification(&mut self, notification: NotificationIn<H>) {
        if self.tx_consensus.unbounded_send(notification).is_err() {
            warn!(target: "Stance-runway", "{:?} Channel to consensus should be open", self.index());
            self.exiting = true;
        }
    }
    ...
    fn status_report(&self) {
        ...
        info!(target: "Stance-runway", "{}", runway_status);
    }
    ...
    async fn run(
        ...
        debug!(target: "Stance-runway", "{:?} Runway started.", index);
        loop {
            futures::select! {
                notification = self.rx_consensus.next() => match notification {
                    Some(notification) => self.on_consensus_notification(notification),
                    None => {
                        error!(target: "Stance-runway", "{:?} Consensus notification stream closed.", index);
                        break;
                    }
                },
                ...
                signed_unit = self.signed_units_from_packer.next() => match signed_unit {
                    ...
                    None => {
                        error!(target: "Stance-runway", "{:?} Packer stream closed.", index);
                        break;
                    }
                },
                ...
                notification = self.notifications_from_alerter.next() => match notification {
                    ...
                    None => {
                        error!(target: "Stance-runway", "{:?} Alert notification stream closed.", index);
                        break;
                    }
                },
                ...
                event = self.unit_messages_from_network.next() => match event {
                    ...
                    None => {
                        error!(target: "Stance-runway", "{:?} Unit message stream closed.", index);
                        break;
                    }
                },
                ...
                message = units_from_backup => match message {
                    ...
                    Err(e) => {
                        error!(target: "Stance-runway", "{:?} Units message from backup channel closed: {:?}", index, e);
                        break;
                    },
                },
                ...
                batch = self.ordered_batch_rx.next() => match batch {
                    ...
                    None => {
                        error!(target: "Stance-runway", "{:?} Ordered batch stream closed.", index);
                        break;
                    }
                },
                ...
                _ = &mut terminator.get_exit() => {
                    debug!(target: "Stance-runway", "{:?} received exit signal", index);
                    self.exiting = true;
                }
            };
            ...
            if self.exiting {
                debug!(target: "Stance-runway", "{:?} Runway decided to exit.", index);
                terminator.terminate_sync().await;
                break;
            }
        }

        debug!(target: "Stance-runway", "{:?} Run ended.", index);
    }
}
...
fn initial_unit_collection<'a, H: Hasher, D: Data, MK: MultiKeychain>(
    ...
    if let Err(e) = unit_messages_for_network.unbounded_send(notification) {
        error!(target: "Stance-runway", "Unable to send the newest unit request: {}", e);
        return Err(());
    };
...
fn trivial_start(
    ...
    if let Err(e) = starting_round_sender.send(0) {
        error!(target: "Stance-runway", "Unable to send the starting round: {}", e);
        return Err(());
    }
    Ok(async {})
}
    ...
    SH: SpawnHandle,
{
    ...
    let alerter_terminator = terminator.add_offspring_connection("Stance-alerter");
    ...
    let consensus_terminator = terminator.add_offspring_connection("Stance-consensus");
    ...
    let packer_handle = spawn_handle
        .spawn_essential("runway/packer", {
            let packer_terminator = terminator.add_offspring_connection("Stance-packer");
            ...
            async move {
                match packer.run(packer_terminator).await {
                    Ok(()) => (),
                    Err(()) => {
                        debug!(target: "Stance-runway", "{:?} Packer task terminated abnormally", index)
                    }
                }
            }
        })
    ...
    loop {
        futures::select! {
            _ = runway_handle => {
                debug!(target: "Stance-runway", "{:?} Runway task terminated early.", index);
                break;
            },
            _ = alerter_handle => {
                debug!(target: "Stance-runway", "{:?} Alerter task terminated early.", index);
                break;
            },
            _ = consensus_handle => {
                debug!(target: "Stance-runway", "{:?} Consensus task terminated early.", index);
                break;
            },
            _ = packer_handle => {
                debug!(target: "Stance-runway", "{:?} Packer task terminated early.", index);
                break;
            },
            _ = starting_round_handle => {
                debug!(target: "Stance-runway", "{:?} Starting round task terminated.", index);
            },
            _ = backup_loading_handle => {
                debug!(target: "Stance-runway", "{:?} Backup loading task terminated.", index);
            },
            _ = &mut terminator.get_exit() => {
                break;
            }
    ...
    debug!(target: "Stance-runway", "{:?} Ending run.", index);
    terminator.terminate_sync().await;

    handle_task_termination(consensus_handle, "Stance-runway", "Consensus", index).await;
    handle_task_termination(alerter_handle, "Stance-runway", "Alerter", index).await;
    handle_task_termination(runway_handle, "Stance-runway", "Runway", index).await;
    handle_task_termination(packer_handle, "Stance-runway", "Packer", index).await;

    debug!(target: "Stance-runway", "{:?} Runway ended.", index);
}
