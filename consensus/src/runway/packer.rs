    ...
    MK: MultiKeychain,
{
    ...
    async fn pack(&mut self) {
        loop {
            ...
            let preunit = match self.preunits_from_runway.next().await {
                ...
                None => {
                    error!(target: "Stance-packer", "{:?} Runway PreUnit stream closed.", self.index());
                    break;
                }
            };
            debug!(target: "Stance-packer", "{:?} Received PreUnit.", self.index());
            let data = self.data_provider.get_data().await;
            debug!(target: "Stance-packer", "{:?} Received data.", self.index());
            ...
            if self
                ...
                .is_err()
            {
                error!(target: "Stance-packer", "{:?} Could not send SignedUnit to Runway.", self.index());
                break;
            }
        }
    }
    ...
    pub async fn run(&mut self, mut terminator: Terminator) -> Result<(), ()> {
        debug!(target: "Stance-packer", "{:?} Packer started.", self.index());
...
mod tests {
    ...
    fn prepare(
        ...
        (
            ...
            packer,
            exit_tx,
            Terminator::create_root(exit_rx, "Stance-packer"),
            preunit,
        )
    }
    ...
    async fn handles_requests_concurrently() {
        ...
        let control_hash = ControlHash::new(&parent_map);
        let preunit = PreUnit::new(NODE_ID, 0, control_hash);
        let packer_handle = packer.run(Terminator::create_root(exit_rx, "Stance-packer"));
        for _ in 0..3 {
            preunits_channel
                .unbounded_send(preunit.clone())
                .expect("Packer PreUnit channel closed");
