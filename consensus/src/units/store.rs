...
impl<H: Hasher, D: Data, K: Keychain> UnitStore<H, D, K> {
    ...
    pub(crate) fn mark_forker(&mut self, forker: NodeIndex) -> Vec<SignedUnit<H, D, K>> {
        if self.is_forker[forker] {
            warn!(target: "Stance-unit-store", "Trying to mark the node {:?} as forker for the second time.", forker);
        }
    ...
    pub(crate) fn add_unit(&mut self, su: SignedUnit<H, D, K>, alert: bool) {
        ...
        if alert {
            trace!(target: "Stance-unit-store", "Adding unit with alert {:?}.", su.as_signable());
            assert!(
        ...
        if self.contains_hash(&hash) {
            ...
            trace!(target: "Stance-unit-store", "A unit ignored as a duplicate {:?}.", su.as_signable());
            return;
        }
