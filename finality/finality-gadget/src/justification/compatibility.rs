...
use crate::{
    ...
    justification::DagestanJustification,
    Version,
};
...
struct DagestanJustificationV1 {
    pub signature: SignatureSet<SignatureV1>,
}

impl From<DagestanJustificationV1> for DagestanJustification {
    fn from(justification: DagestanJustificationV1) -> DagestanJustification {
        ...
        DagestanJustification::CommitteeMultisignature(just_drop_id)
    }
}
...
#[derive(Clone, Encode, Decode, Debug, PartialEq, Eq)]
struct DagestanJustificationV2 {
    pub signature: SignatureSet<Signature>,
}

impl From<DagestanJustificationV2> for DagestanJustification {
    fn from(justification: DagestanJustificationV2) -> DagestanJustification {
        DagestanJustification::CommitteeMultisignature(justification.signature)
    }
}

#[derive(Clone, Debug, PartialEq, Eq)]
enum VersionedDagestanJustification {
    // Most likely from the future.
    Other(Version, Vec<u8>),
    V1(DagestanJustificationV1),
    V2(DagestanJustificationV2),
    V3(DagestanJustification),
}
...
fn encode_with_version(version: Version, payload: &[u8]) -> Vec<u8> {
    ...
    // that big.
    // We do not have a guarantee that size_hint is implemented for DagestanJustification, so we need
...
impl Encode for VersionedDagestanJustification {
    fn size_hint(&self) -> usize {
        use VersionedDagestanJustification::*;
...
impl Decode for VersionedDagestanJustification {
    fn decode<I: CodecInput>(input: &mut I) -> Result<Self, CodecError> {
        ...
        match version {
            Version(1) => Ok(V1(DagestanJustificationV1::decode(input)?)),
            Version(2) => Ok(V2(DagestanJustificationV2::decode(input)?)),
            Version(3) => Ok(V3(DagestanJustification::decode(input)?)),
            _ => {
...
fn decode_pre_compatibility_justification(
...
) -> Result<DagestanJustification, Error> {
    use Error::*;
    ...
    match DagestanJustificationV2::decode_all(&mut justification_cloned.as_slice()) {
        Ok(justification) => Ok(justification.into()),
        Err(_) => match DagestanJustificationV1::decode_all(&mut justification_raw.as_slice()) {
            Ok(justification) => Ok(justification.into()),
            Err(_) => Err(BadFormat),
        },
    }
...
pub fn backwards_compatible_decode(
    ...
    use Error::*;
    let justification_cloned = justification_raw.clone();
    match VersionedDagestanJustification::decode_all(&mut justification_cloned.as_slice()) {
        Ok(justification) => {
            use VersionedDagestanJustification::*;
            match justification {
                ...
                Other(version, _) => {
                    ...
                    // which is in VersionedDagestanJustification byte_count_size, can be small enough
...
pub fn versioned_encode(justification: DagestanJustification) -> Vec<u8> {
    VersionedDagestanJustification::V3(justification).encode()
}

#[cfg(test)]
mod test {
    use dagestan_primitives::{AuthorityPair, AuthoritySignature};
    use codec::{Decode, Encode};
    use sp_core::Pair;

    use super::{
        backwards_compatible_decode, versioned_encode, DagestanJustificationV1, DagestanJustificationV2,
        VersionedDagestanJustification,
    };
    use crate::{
        crypto::{Signature, SignatureV1},
        justification::DagestanJustification,
    ...
    fn correctly_decodes_v2() {
        ...
        let just_v2 = DagestanJustificationV2 {
            signature: signature_set,
        };
        let encoded_just: Vec<u8> = just_v2.encode();
        let decoded = backwards_compatible_decode(encoded_just);
        let just_v2: DagestanJustification = just_v2.into();
    ...
    fn correctly_decodes_v3_committee() {
        ...
        let just_v3 = DagestanJustification::CommitteeMultisignature(signature_set);
    ...
    fn correctly_decodes_other() {
        let other = VersionedDagestanJustification::Other(Version(43), vec![21, 37]);
        let encoded = other.encode();
        let decoded = VersionedDagestanJustification::decode(&mut encoded.as_slice());
    ...
    fn assert_backwards_compatible_decodes_pre_compatibility_justification(
        ...
        match backwards_compatible_decode(raw_justification_legacy_pre_compatibility) {
            Ok(DagestanJustification::CommitteeMultisignature(signature)) => {
                assert_eq!(signature.size(), NodeCount(expected_node_count))
            }
            Ok(DagestanJustification::EmergencySignature(_)) => {
