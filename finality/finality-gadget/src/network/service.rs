...
impl<N: Network, D: Data> Service<N, D> {
                ...
                if let Some(data) = receiver.next().await {
                    let sender = if let Some(sender) = senders.get(&protocol) {
                        ...
                        match network.sender(peer_id.clone(), protocol) {
                            ...
                            Err(e) => {
                                debug!(target: "stance-network", "Failed creating sender. Dropping message: {}", e);
                                continue;
                            }
                        }
                    };
                    if let Err(e) = sender.send(data.encode()).await {
                        debug!(target: "stance-network", "Failed sending data to peer. Dropping sender and message: {}", e);
                        senders.remove(&protocol);
                    }
                } else {
                    debug!(target: "stance-network", "Sender was dropped for peer {:?}. Peer sender exiting.", peer_id);
                    return;
                }
            }
        }
                ...
                match sender.unbounded_send(data) {
                    Err(e) => {
                        ...
                        if e.is_disconnected() {
                            trace!(target: "stance-network", "Failed sending data to peer because peer_sender receiver is dropped: {:?}", peer);
                        }
    ...
    fn broadcast(&mut self, data: D) {
        for peer in self.generic_connected_peers.clone() {
            ...
            if let Err(e) = self.send_to_peer(data.clone(), peer.clone(), Protocol::Generic) {
                trace!(target: "stance-network", "Failed to send broadcast to peer{:?}, {:?}", peer, e);
            }
        }
    }
    ...
    fn handle_network_event(
        ...
        match event {
            Connected(multiaddress) => {
                trace!(target: "stance-network", "Connected event from address {:?}", multiaddress);
                self.network
                    .add_reserved(iter::once(multiaddress).collect(), Protocol::Generic);
            }
            Disconnected(peer) => {
                trace!(target: "stance-network", "Disconnected event for peer {:?}", peer);
                self.network
                    .remove_reserved(iter::once(peer).collect(), Protocol::Generic);
            }
            StreamOpened(peer, protocol) => {
                trace!(target: "stance-network", "StreamOpened event for peer {:?} and the protocol {:?}.", peer, protocol);
                ...
                self.spawn_handle.spawn(
                    "stance/network/peer_sender",
                    None,
                    self.peer_sender(peer, rx, protocol),
                );
            }
            StreamClosed(peer, protocol) => {
                trace!(target: "stance-network", "StreamClosed event for peer {:?} and protocol {:?}", peer, protocol);
                match protocol {
                    Protocol::Generic => {
            ...
            Messages(messages) => {
                for data in messages.into_iter() {
                    match D::decode(&mut &data[..]) {
                        ...
                        Err(e) => warn!(target: "stance-network", "Error decoding message: {}", e),
                    }
                }
            }
        }
        ...
        match command {
            ...
            SendTo(peer, protocol) => {
                if let Err(e) = self.send_to_peer(data, peer.clone(), protocol) {
                    trace!(target: "stance-network", "Failed to send data to peer{:?} via protocol {:?}, {:?}", peer, protocol, e);
                }
            }
        }
    }
    ...
    fn status_report(&self) {
        ...
        info!(target: "stance-network", "{}", status);
    }
            ...
            tokio::select! {
                maybe_event = events_from_network.next_event() => match maybe_event {
                    Some(event) => if let Err(e) = self.handle_network_event(event) {
                        error!(target: "stance-network", "Cannot forward messages to user: {:?}", e);
                        return;
                    },
                    None => {
                        error!(target: "stance-network", "Network event stream ended.");
                        return;
                    }
                },
                maybe_command = self.commands_from_manager.next() => match maybe_command {
                    ...
                    None => {
                        error!(target: "stance-network", "Manager command stream ended.");
                        return;
                    }
                },
                maybe_message = self.messages_from_user.next() => match maybe_message {
                    ...
                    None => {
                        error!(target: "stance-network", "User message stream ended.");
                        return;
                    }
                },
