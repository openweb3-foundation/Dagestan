...
impl<NI: NetworkIdentity, D: Data> Service<NI, D> {
    ...
    fn addresses(&self) -> Vec<NI::Multiaddress> {
        ...
        debug!(target: "stance-network", "Got addresses:\n{:?}\n and peer_id:{:?}", addresses, peer_id);
        addresses
            .into_iter()
            ...
            Ok((actions, data_from_network)) => {
                if let Some(result_for_user) = result_for_user {
                    if result_for_user.send(data_from_network).is_err() {
                        warn!(target: "stance-network", "Failed to send started session.")
                    }
                }
                Ok(actions)
            }
        ...
        match self.sessions.get_mut(&session_id) {
            Some(Session {
                ...
                let maybe_command = match !addresses.is_empty() && handler.is_validator() {
                    true => {
                        debug!(target: "stance-network", "Adding addresses for session {:?} to reserved: {:?}", session_id, addresses);
            ...
            None => {
                debug!(target: "stance-network", "Received message from unknown session: {:?}", message);
                ServiceActions::noop()
            }
        }
    }
...
impl<D: Data, M: Multiaddress> IO<D, M> {
    ...
    pub async fn run<NI: NetworkIdentity<Multiaddress = M, PeerId = M::PeerId>>(
        ...
        loop {
            trace!(target: "stance-network", "Manager Loop started a next iteration");
            tokio::select! {
                maybe_command = self.commands_from_user.next() => {
                    trace!(target: "stance-network", "Manager received a command from user");
                    match maybe_command {
                        Some(command) => match service.on_command(command).await {
                            Ok(to_send) => self.send(to_send)?,
                            Err(e) => warn!(target: "stance-network", "Failed to update handler: {:?}", e),
                        },
                        None => return Err(Error::CommandsChannel),
                    }
                },
                maybe_message = self.messages_from_user.next() => {
                    trace!(target: "stance-network", "Manager received a message from user");
                    match maybe_message {
                ...
                maybe_message = self.messages_from_network.next() => {
                    trace!(target: "stance-network", "Manager received a message from network");
                    match maybe_message {
                        Some(message) => if let Err(e) = self.on_network_message(&mut service, message) {
                            match e {
                                Error::UserSend => trace!(target: "stance-network", "Failed to send to user in session."),
                                Error::NoSession => trace!(target: "stance-network", "Received message for unknown session."),
                                _ => return Err(e),
                            }
                        },
                ...
                _ = maintenance.tick() => {
                    debug!(target: "stance-network", "Manager starts maintenence");
                    match service.retry_session_start().await {
                        Ok(to_send) => self.send(to_send)?,
                        Err(e) => warn!(target: "stance-network", "Retry failed to update handler: {:?}", e),
                    }
