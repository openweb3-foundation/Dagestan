...
cargo build --release
binary="../../target/release/stance-examples-ordering"
...
for id in $(seq 0 $(expr $N_NODES + $N_MALFUNCTIONING_NODES - 1)); do
    rm -f "stance-examples-ordering-backup/${id}.units"
    rm -f "node${id}.log"
done


echo "WARNING
The current implementation of Stance does not strictly guarantee
