...
use stance_consensus_mock::{Keychain, Spawner};
use clap::Parser;
...
fn create_backup(node_id: NodeIndex) -> Result<(File, io::Cursor<Vec<u8>>), io::Error> {
    let stash_path = Path::new("./stance-examples-ordering-backup");
...
async fn main() {
    ...
    let member_terminator = Terminator::create_root(exit_rx, "Stance-member");
    let member_handle = tokio::spawn(async move {
        let keychain = Keychain::new(n_members, id);
        let config = stance_consensus::default_config(n_members, id, 0);
        run_session(
            config,
            local_io,
            network,
