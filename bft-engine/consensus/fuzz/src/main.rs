...
fn main() {
    ...
    if opt.check_fuzz {
        stance_bft_fuzz::check_fuzz(BufReader::new(io::stdin()), opt.members, Some(opt.batches));
    } else {
        stance_bft_fuzz::generate_fuzz(BufWriter::new(io::stdout()), opt.members, opt.batches);
    }
}
