...
/// to the Terminal. We refer to the documentation https://setheum-labs.github.io/Stance/internals.html
...
impl<H: Hasher, D: Data, K: Keychain> UnitStore<H, D, K> {
    ...
    pub(crate) fn mark_forker(&mut self, forker: NodeIndex) -> Vec<SignedUnit<H, D, K>> {
        if self.is_forker[forker] {
            warn!(target: "StanceBFT-unit-store", "Trying to mark the node {:?} as forker for the second time.", forker);
        }
    ...
    pub(crate) fn add_unit(&mut self, su: SignedUnit<H, D, K>, alert: bool) {
        ...
        if alert {
            trace!(target: "StanceBFT-unit-store", "Adding unit with alert {:?}.", su.as_signable());
            assert!(
        ...
        if self.contains_hash(&hash) {
            ...
            trace!(target: "StanceBFT-unit-store", "A unit ignored as a duplicate {:?}.", su.as_signable());
            return;
        }
...
mod tests {
    ...
    use stance_bft_mock::{Data, Hasher64, Keychain};
