...
impl<'a, H: Hasher, D: Data, MK: MultiKeychain> IO<'a, H, D, MK> {
    ...
    pub fn send_notification_for_units(
        ...
        if self
            ...
            .is_err()
        {
            warn!(target: "StanceBFT-alerter", "{:?} Channel with forking notifications should be open", self.alerter_index);
            *exiting = true;
        }
    }
    ...
    pub fn send_message_for_network(
        ...
        if self
            ...
            .is_err()
        {
            warn!(target: "StanceBFT-alerter", "{:?} Channel with notifications for network should be open", self.alerter_index);
            *exiting = true;
        }
    }
}
