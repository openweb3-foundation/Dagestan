...
use stance_bft_rmc::{DoublingDelayScheduler, Message as RmcMessage, ReliableMulticast};
...
/// The component responsible for fork alerts in StanceBFT. We refer to the documentation
/// https://setheum-labs.github.io/Stance/how_stancebft_does_it.html Section 2.5 and
/// https://setheum-labs.github.io/Stance/reliable_broadcast.html for a discussion.
...
impl<'a, H: Hasher, D: Data, MK: MultiKeychain> Alerter<'a, H, D, MK> {
    ...
    fn correct_commitment(
        ...
        for u in units {
            let u = match u.clone().check(self.keychain) {
                Ok(u) => u,
                Err(_) => {
                    warn!(target: "StanceBFT-alerter", "{:?} One of the units is incorrectly signed.", self.index());
                    return false;
                }
            };
            let full_unit = u.as_signable();
            if full_unit.creator() != forker {
                warn!(target: "StanceBFT-alerter", "{:?} One of the units {:?} has wrong creator.", self.index(), full_unit);
                return false;
            }
            if rounds.contains(&full_unit.round()) {
                warn!(target: "StanceBFT-alerter", "{:?} Two or more alerted units have the same round {:?}.", self.index(), full_unit.round());
                return false;
            }
    ...
    fn who_is_forking(&self, proof: &ForkProof<H, D, MK::Signature>) -> Option<NodeIndex> {
        ...
        let (u1, u2) = {
            ...
            match (u1, u2) {
                (Ok(u1), Ok(u2)) => (u1, u2),
                _ => {
                    warn!(target: "StanceBFT-alerter", "{:?} Invalid signatures in a proof.", self.index());
                    return None;
                }
            }
        };
        ...
        {
            warn!(target: "StanceBFT-alerter", "{:?} Alert from different session.", self.index());
            return None;
        }
        if full_unit1 == full_unit2 {
            warn!(target: "StanceBFT-alerter", "{:?} Two copies of the same unit do not constitute a fork.", self.index());
            return None;
        }
        if full_unit1.creator() != full_unit2.creator() {
            warn!(target: "StanceBFT-alerter", "{:?} One of the units creators in proof does not match.", self.index());
            return None;
        }
        if full_unit1.round() != full_unit2.round() {
            warn!(target: "StanceBFT-alerter", "{:?} The rounds in proof's units do not match.", self.index());
            return None;
        }
    ...
    fn on_network_alert(
        ...
        let alert = match alert.check(self.keychain) {
            Ok(alert) => alert,
            Err(e) => {
                warn!(target: "StanceBFT-alerter","{:?} We have received an incorrectly signed alert: {:?}.", self.index(), e);
                return None;
            }
        };
        ...
        if let Some(forker) = self.who_is_forking(&contents.proof) {
            if self.known_rmcs.contains_key(&(contents.sender, forker)) {
                debug!(target: "StanceBFT-alerter","{:?} We already know about an alert by {:?} about {:?}.", self.index(), alert.as_signable().sender, forker);
            ...
            warn!(target: "StanceBFT-alerter","{:?} We have received an incorrect forking proof from {:?}.", self.index(), alert.as_signable().sender);
            None
        }
    }
    ...
    fn on_message(
        ...
        match message {
            ForkAlert(alert) => {
                trace!(target: "StanceBFT-alerter", "{:?} Fork alert received {:?}.", self.index(), alert);
            ...
            AlertRequest(node, hash) => match self.known_alerts.get(&hash) {
                ...
                None => {
                    debug!(target: "StanceBFT-alerter", "{:?} Received request for unknown alert.", self.index());
                    None
                }
            },
        }
    ...
    fn alert_confirmed(
        ...
        let alert = match self.known_alerts.get(multisigned.as_signable()) {
            ...
            None => {
                error!(target: "StanceBFT-alerter", "{:?} Completed an RMC for an unknown alert.", self.index());
                return None;
            }
        };
        ...
        if !self.correct_commitment(forker, &alert.legit_units) {
            warn!(target: "StanceBFT-alerter","{:?} We have received an incorrect unit commitment from {:?}.", self.index(), alert.sender);
            return None;
        }
    ...
    loop {
        futures::select! {
            message = io.messages_from_network.next() => match message {
                    ...
                    match alerter.on_message(message) {
                        ...
                        Some(AlerterResponse::RmcMessage(message)) => {
                            if io.messages_for_rmc.unbounded_send(message).is_err() {
                                warn!(target: "StanceBFT-alerter", "{:?} Channel with messages for rmc should be open", alerter.index());
                ...
                None => {
                    error!(target: "StanceBFT-alerter", "{:?} Message stream closed.", alerter.index());
                    break;
                }
            },
            alert = io.alerts_from_units.next() => match alert {
                ...
                None => {
                    error!(target: "StanceBFT-alerter", "{:?} Alert stream closed.", alerter.index());
                    break;
                }
            },
            message = io.messages_from_rmc.next() => match message {
                ...
                None => {
                    error!(target: "StanceBFT-alerter", "{:?} RMC message stream closed.", alerter.index());
                    break;
                }
            },
            ...
            _ = &mut terminator.get_exit() => {
                debug!(target: "StanceBFT-alerter", "{:?} received exit signal", alerter.index());
                alerter.exiting = true;
            },
        }
        if alerter.exiting {
            debug!(target: "StanceBFT-alerter", "{:?} Alerter decided to exit.", alerter.index());
...
mod tests {
    ...
    use stance_bft_types::{NodeCount, NodeIndex, NodeMap, Signable, Signed};
