    ...
    S: Signature,
{
    ...
    fn on_request_coord(&mut self, coord: UnitCoord) {
        trace!(target: "StanceBFT-member", "{:?} Dealing with missing coord notification {:?}.", self.index(), coord);
    ...
    fn send_unit_message(&mut self, message: UnitMessage<H, D, S>, recipient: Recipient) {
        if self
            ...
            .is_err()
        {
            warn!(target: "StanceBFT-member", "{:?} Channel to network should be open", self.index());
            self.exiting = true;
        }
    }
    ...
    fn status_report(&self) {
        ...
        info!(target: "StanceBFT-member", "{}", status);
    }
        ...
        loop {
            futures::select! {
                event = self.notifications_from_runway.next() => match event {
                    ...
                    None => {
                        error!(target: "StanceBFT-member", "{:?} Unit message stream from Runway closed.", self.index());
                        break;
                    },
                },
                ...
                event = self.resolved_requests.next() => match event {
                    ...
                    None => {
                        error!(target: "StanceBFT-member", "{:?} Resolved-requests stream from Runway closed.", self.index());
                        break;
                    }
                },
                ...
                event = self.unit_messages_from_network.next() => match event {
                    Some(message) => match message.try_into() {
                        ...
                        Err(_) => error!(target: "StanceBFT-member", "{:?} Unable to convert a UnitMessage into an instance of RunwayNotificationIn.", self.index()),
                    },
                    None => {
                        error!(target: "StanceBFT-member", "{:?} Unit message stream from network closed.", self.index());
                        break;
                    },
                },
                ...
                _ = &mut terminator.get_exit() => {
                    debug!(target: "StanceBFT-member", "{:?} received exit signal", self.index());
                    self.exiting = true;
                },
            }
            if self.exiting {
                debug!(target: "StanceBFT-member", "{:?} Member decided to exit.", self.index());
                terminator.terminate_sync().await;
                break;
            }
        }

        debug!(target: "StanceBFT-member", "{:?} Member stopped.", self.index());
    }
    ...
    fn send_notification_to_runway(&mut self, notification: RunwayNotificationIn<H, D, S>) {
        if self
            ...
            .is_err()
        {
            warn!(target: "StanceBFT-member", "{:?} Sender to runway with RunwayNotificationIn messages should be open", self.index());
            self.exiting = true;
        }
    }
}
...
/// [docs for devs](https://setheum-labs.github.io/Stance/index.html).
...
>(
    ...
    info!(target: "StanceBFT-member", "{:?} Starting a new session.", index);
    debug!(target: "StanceBFT-member", "{:?} Spawning party for a session.", index);
    ...
    debug!(target: "StanceBFT-member", "{:?} Spawning network.", index);
    let network_terminator = terminator.add_offspring_connection("StanceBFT-network");
    ...
    debug!(target: "StanceBFT-member", "{:?} Network spawned.", index);

    debug!(target: "StanceBFT-member", "{:?} Initializing Runway.", index);
    let runway_terminator = terminator.add_offspring_connection("StanceBFT-runway");
    ...
    debug!(target: "StanceBFT-member", "{:?} Runway spawned.", index);

    debug!(target: "StanceBFT-member", "{:?} Initializing Member.", index);
    ...
    let member_terminator = terminator.add_offspring_connection("StanceBFT-member");
    ...
    debug!(target: "StanceBFT-member", "{:?} Member initialized.", index);

    futures::select! {
        _ = network_handle => {
            error!(target: "StanceBFT-member", "{:?} Network-hub terminated early.", index);
        },

        _ = runway_handle => {
            error!(target: "StanceBFT-member", "{:?} Runway terminated early.", index);
        },

        _ = member_handle => {
            error!(target: "StanceBFT-member", "{:?} Member terminated early.", index);
        },

        _ = &mut terminator.get_exit() => {
            debug!(target: "StanceBFT-member", "{:?} exit channel was called.", index);
        },
    }

    debug!(target: "StanceBFT-member", "{:?} Run ending.", index);

    terminator.terminate_sync().await;

    handle_task_termination(network_handle, "StanceBFT-member", "Network", index).await;
    handle_task_termination(runway_handle, "StanceBFT-member", "Runway", index).await;
    handle_task_termination(member_handle, "StanceBFT-member", "Member", index).await;

    info!(target: "StanceBFT-member", "{:?} Session ended.", index);
}
...
mod tests {
    ...
    use stance_bft_mock::{Hasher64, Signature};
    use stance_bft_types::NodeCount;
