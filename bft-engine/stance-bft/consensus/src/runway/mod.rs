    ...
    MK: MultiKeychain,
{
    ...
    async fn on_unit_message(&mut self, message: RunwayNotificationIn<H, D, MK::Signature>) {
        match message {
            RunwayNotificationIn::NewUnit(u) => {
                trace!(target: "StanceBFT-runway", "{:?} New unit received {:?}.", self.index(), &u);
                self.on_unit_received(u, false)
            }

            RunwayNotificationIn::Request(request, node_id) => match request {
                Request::Coord(coord) => {
                    trace!(target: "StanceBFT-runway", "{:?} Coords request received {:?}.", self.index(), coord);
                    self.on_request_coord(node_id, coord)
                }
                Request::Parents(u_hash) => {
                    trace!(target: "StanceBFT-runway", "{:?} Parents request received {:?}.", self.index(), u_hash);
                    self.on_request_parents(node_id, u_hash)
                }
                Request::NewestUnit(salt) => {
                    trace!(target: "StanceBFT-runway", "{:?} Newest unit request received {:?}.", self.index(), salt);
            ...
            RunwayNotificationIn::Response(res) => match res {
                Response::Coord(u) => {
                    trace!(target: "StanceBFT-runway", "{:?} Fetch response received {:?}.", self.index(), &u);
                    self.on_unit_received(u, false)
                }
                Response::Parents(u_hash, parents) => {
                    trace!(target: "StanceBFT-runway", "{:?} Response parents received {:?}.", self.index(), u_hash);
                    self.on_parents_response(u_hash, parents)
                }
                Response::NewestUnit(response) => {
                    trace!(target: "StanceBFT-runway", "{:?} Response newest unit received from {:?}.", self.index(), response.index());
                    let res = self.responses_for_collection.unbounded_send(response);
                    if res.is_err() {
                        debug!(target: "StanceBFT-runway", "{:?} Could not send response to collection ({:?}).", self.index(), res)
                    }
                }
            },
        }
    ...
    fn on_unit_received(&mut self, uu: UncheckedSignedUnit<H, D, MK::Signature>, alert: bool) {
        match self.validator.validate_unit(uu) {
            ...
            Err(e) => warn!(target: "StanceBFT-member", "Received unit failing validation: {}", e),
        }
    }
    ...
    fn add_unit_to_store_unless_fork(&mut self, su: SignedUnit<H, D, MK>) {
        ...
        trace!(target: "StanceBFT-member", "{:?} Adding member unit to store {:?}", self.index(), full_unit);
        if self.store.is_forker(full_unit.creator()) {
            trace!(target: "StanceBFT-member", "{:?} Ignoring forker's unit {:?}", self.index(), full_unit);
            return;
        }
    ...
    fn on_new_forker_detected(&mut self, forker: NodeIndex, proof: ForkProof<H, D, MK::Signature>) {
        ...
        if self.alerts_for_alerter.unbounded_send(alert).is_err() {
            warn!(target: "StanceBFT-runway", "{:?} Channel to alerter should be open", self.index());
            self.exiting = true;
        }
    }
    ...
    fn on_request_coord(&mut self, node_id: NodeIndex, coord: UnitCoord) {
        debug!(target: "StanceBFT-runway", "{:?} Received fetch request for coord {:?} from {:?}.", self.index(), coord, node_id);
        let maybe_su = (self.store.unit_by_coord(coord)).cloned();

        if let Some(su) = maybe_su {
            trace!(target: "StanceBFT-runway", "{:?} Answering fetch request for coord {:?} from {:?}.", self.index(), coord, node_id);
            self.send_message_for_network(RunwayNotificationOut::Response(
                Response::Coord(su.into()),
                node_id,
            ));
        } else {
            trace!(target: "StanceBFT-runway", "{:?} Not answering fetch request for coord {:?}. Unit not in store.", self.index(), coord);
        }
    }

    fn on_request_parents(&mut self, node_id: NodeIndex, u_hash: H::Hash) {
        debug!(target: "StanceBFT-runway", "{:?} Received parents request for hash {:?} from {:?}.", self.index(), u_hash, node_id);

        if let Some(p_hashes) = self.store.get_parents(u_hash) {
            let p_hashes = p_hashes.clone();
            trace!(target: "StanceBFT-runway", "{:?} Answering parents request for hash {:?} from {:?}.", self.index(), u_hash, node_id);
            ...
            for hash in p_hashes.iter() {
                if let Some(fu) = self.store.unit_by_hash(hash) {
                    ...
                    debug!(target: "StanceBFT-runway", "{:?} Not answering parents request, one of the parents missing from store.", self.index());
            ...
            trace!(target: "StanceBFT-runway", "{:?} Not answering parents request for hash {:?}. Unit not in DAG yet.", self.index(), u_hash);
        }
    }
    ...
    async fn on_request_newest(&mut self, requester: NodeIndex, salt: u64) {
        ...
        {
            error!(target: "StanceBFT-runway", "Unable to send response to network: {}", e);
        }
    }
    ...
    fn on_parents_response(
        ...
        if self.store.get_parents(u_hash).is_some() {
            trace!(target: "StanceBFT-runway", "{:?} We got parents response but already know the parents.", self.index());
            return;
        }
        let (u_round, u_control_hash, parent_ids) = match self.store.unit_by_hash(&u_hash) {
            ...
            None => {
                trace!(target: "StanceBFT-runway", "{:?} We got parents but don't even know the unit. Ignoring.", self.index());
                return;
            }
        };

        if parent_ids.len() != parents.len() {
            warn!(target: "StanceBFT-runway", "{:?} In received parent response expected {} parents got {} for unit {:?}.", self.index(), parents.len(), parent_ids.len(), u_hash);
            return;
        }
        ...
        for (i, uu) in parents.into_iter().enumerate() {
            let su = match self.validator.validate_unit(uu) {
                Ok(su) => su,
                Err(e) => {
                    warn!(target: "StanceBFT-runway", "{:?} In received parent response received a unit that does not pass validation: {}", self.index(), e);
                    return;
                }
            };
            let full_unit = su.as_signable();
            if full_unit.round() + 1 != u_round {
                warn!(target: "StanceBFT-runway", "{:?} In received parent response received a unit with wrong round.", self.index());
                return;
            }
            if full_unit.creator() != parent_ids[i] {
                warn!(target: "StanceBFT-runway", "{:?} In received parent response received a unit with wrong creator.", self.index());
                return;
            }
        ...
        if ControlHash::<H>::combine_hashes(&p_hashes_node_map) != u_control_hash {
            warn!(target: "StanceBFT-runway", "{:?} In received parent response the control hash is incorrect {:?}.", self.index(), p_hashes_node_map);
            return;
        }
        let p_hashes: Vec<_> = p_hashes_node_map.into_values().collect();
        self.store.add_parents(u_hash, p_hashes.clone());
        trace!(target: "StanceBFT-runway", "{:?} Succesful parents response for {:?}.", self.index(), u_hash);
    ...
    fn save_unit(&mut self, uu: UncheckedSignedUnit<H, D, MK::Signature>) {
        ...
        trace!(target: "StanceBFT-runway", "{:?} Saving a created unit {:?}.", self.index(), h);
        if let Err(err) = self.unit_saver.save(uu) {
            error!(target: "StanceBFT-runway", "{:?} Failed to save unit {:?}. {}", self.index(), h, err);
        } else {
            trace!(target: "StanceBFT-runway", "{:?} Saved a created unit {:?}.", self.index(), h);
        }
    }

    fn on_packed(&mut self, signed_unit: SignedUnit<H, D, MK>) {
        debug!(target: "StanceBFT-runway", "{:?} On create notification.", self.index());
        ...
        match notification {
            NotificationOut::CreatedPreUnit(pu, _) => {
                if self.preunits_for_packer.unbounded_send(pu).is_err() {
                    warn!(target: "StanceBFT-runway", "{:?} preunits_for_packer channel should be open", self.index());
                    self.exiting = true;
                }
            }
            ...
            NotificationOut::AddedToDag(h, p_hashes) => {
                ...
                if let Some(su) = self.store.unit_by_hash(&h).cloned() {
                    ...
                    if su.as_signable().creator() == self.index() {
                        trace!(target: "StanceBFT-runway", "{:?} Sending a unit {:?}.", self.index(), h);
                        self.send_message_for_network(RunwayNotificationOut::NewSelfUnit(
                            su.into(),
                        ));
                    }
                } else {
                    error!(target: "StanceBFT-runway", "{:?} A unit already added to DAG is not in our store: {:?}.", self.index(), h);
                }
            }
        }
    }
    ...
    fn on_missing_coords(&mut self, mut coords: Vec<UnitCoord>) {
        trace!(target: "StanceBFT-runway", "{:?} Dealing with missing coords notification {:?}.", self.index(), coords);
    ...
    fn on_wrong_control_hash(&mut self, u_hash: H::Hash) {
        trace!(target: "StanceBFT-runway", "{:?} Dealing with wrong control hash notification {:?}.", self.index(), u_hash);
        if let Some(p_hashes) = self.store.get_parents(u_hash) {
            // We have the parents by some strange reason (someone sent us parents
            // without us requesting them).
            let p_hashes = p_hashes.clone();
            trace!(target: "StanceBFT-runway", "{:?} We have the parents for {:?} even though we did not request them.", self.index(), u_hash);
    ...
    fn send_message_for_network(
        ...
        if self
            ...
            .is_err()
        {
            warn!(target: "StanceBFT-runway", "{:?} unit_messages_for_network channel should be open", self.index());
            self.exiting = true;
        }
    }
    ...
    fn send_resolved_request_notification(&mut self, notification: Request<H>) {
        if self.resolved_requests.unbounded_send(notification).is_err() {
            warn!(target: "StanceBFT-runway", "{:?} resolved_requests channel should be open", self.index());
            self.exiting = true;
        }
    }
    ...
    fn send_consensus_notification(&mut self, notification: NotificationIn<H>) {
        if self.tx_consensus.unbounded_send(notification).is_err() {
            warn!(target: "StanceBFT-runway", "{:?} Channel to consensus should be open", self.index());
            self.exiting = true;
        }
    }
    ...
    fn status_report(&self) {
        ...
        info!(target: "StanceBFT-runway", "{}", runway_status);
    }
    ...
    async fn run(
        ...
        debug!(target: "StanceBFT-runway", "{:?} Runway started.", index);
        loop {
            futures::select! {
                notification = self.rx_consensus.next() => match notification {
                    Some(notification) => self.on_consensus_notification(notification),
                    None => {
                        error!(target: "StanceBFT-runway", "{:?} Consensus notification stream closed.", index);
                        break;
                    }
                },
                ...
                signed_unit = self.signed_units_from_packer.next() => match signed_unit {
                    ...
                    None => {
                        error!(target: "StanceBFT-runway", "{:?} Packer stream closed.", index);
                        break;
                    }
                },
                ...
                notification = self.notifications_from_alerter.next() => match notification {
                    ...
                    None => {
                        error!(target: "StanceBFT-runway", "{:?} Alert notification stream closed.", index);
                        break;
                    }
                },
                ...
                event = self.unit_messages_from_network.next() => match event {
                    ...
                    None => {
                        error!(target: "StanceBFT-runway", "{:?} Unit message stream closed.", index);
                        break;
                    }
                },
                ...
                message = units_from_backup => match message {
                    ...
                    Err(e) => {
                        error!(target: "StanceBFT-runway", "{:?} Units message from backup channel closed: {:?}", index, e);
                        break;
                    },
                },
                ...
                batch = self.ordered_batch_rx.next() => match batch {
                    ...
                    None => {
                        error!(target: "StanceBFT-runway", "{:?} Ordered batch stream closed.", index);
                        break;
                    }
                },
                ...
                _ = &mut terminator.get_exit() => {
                    debug!(target: "StanceBFT-runway", "{:?} received exit signal", index);
                    self.exiting = true;
                }
            };
            ...
            if self.exiting {
                debug!(target: "StanceBFT-runway", "{:?} Runway decided to exit.", index);
                terminator.terminate_sync().await;
                break;
            }
        }

        debug!(target: "StanceBFT-runway", "{:?} Run ended.", index);
    }
}
...
fn initial_unit_collection<'a, H: Hasher, D: Data, MK: MultiKeychain>(
    ...
    if let Err(e) = unit_messages_for_network.unbounded_send(notification) {
        error!(target: "StanceBFT-runway", "Unable to send the newest unit request: {}", e);
        return Err(());
    };
...
fn trivial_start(
    ...
    if let Err(e) = starting_round_sender.send(0) {
        error!(target: "StanceBFT-runway", "Unable to send the starting round: {}", e);
        return Err(());
    }
    Ok(async {})
}
    ...
    SH: SpawnHandle,
{
    ...
    let alerter_terminator = terminator.add_offspring_connection("StanceBFT-alerter");
    ...
    let consensus_terminator = terminator.add_offspring_connection("StanceBFT-consensus");
    ...
    let packer_handle = spawn_handle
        .spawn_essential("runway/packer", {
            let packer_terminator = terminator.add_offspring_connection("StanceBFT-packer");
            ...
            async move {
                match packer.run(packer_terminator).await {
                    Ok(()) => (),
                    Err(()) => {
                        debug!(target: "StanceBFT-runway", "{:?} Packer task terminated abnormally", index)
                    }
                }
            }
        })
    ...
    loop {
        futures::select! {
            _ = runway_handle => {
                debug!(target: "StanceBFT-runway", "{:?} Runway task terminated early.", index);
                break;
            },
            _ = alerter_handle => {
                debug!(target: "StanceBFT-runway", "{:?} Alerter task terminated early.", index);
                break;
            },
            _ = consensus_handle => {
                debug!(target: "StanceBFT-runway", "{:?} Consensus task terminated early.", index);
                break;
            },
            _ = packer_handle => {
                debug!(target: "StanceBFT-runway", "{:?} Packer task terminated early.", index);
                break;
            },
            _ = starting_round_handle => {
                debug!(target: "StanceBFT-runway", "{:?} Starting round task terminated.", index);
            },
            _ = backup_loading_handle => {
                debug!(target: "StanceBFT-runway", "{:?} Backup loading task terminated.", index);
            },
            _ = &mut terminator.get_exit() => {
                break;
            }
    ...
    debug!(target: "StanceBFT-runway", "{:?} Ending run.", index);
    terminator.terminate_sync().await;

    handle_task_termination(consensus_handle, "StanceBFT-runway", "Consensus", index).await;
    handle_task_termination(alerter_handle, "StanceBFT-runway", "Alerter", index).await;
    handle_task_termination(runway_handle, "StanceBFT-runway", "Runway", index).await;
    handle_task_termination(packer_handle, "StanceBFT-runway", "Packer", index).await;

    debug!(target: "StanceBFT-runway", "{:?} Runway ended.", index);
}
