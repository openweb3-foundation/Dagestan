...
impl<'a, MK: Keychain> Collection<'a, MK> {
    ...
    pub fn on_newest_response<H: Hasher, D: Data>(
        ...
        if current_round != round {
            debug!(target: "StanceBFT-runway", "Node {} responded with starting unit {}, but now says {}", response.responder.0, current_round, round);
        }
...
impl<'a, H: Hasher, D: Data, MK: Keychain> IO<'a, H, D, MK> {
    ...
    fn finish(self, round: Round) {
        if self.round_for_creator.send(round).is_err() {
            error!(target: "StanceBFT-runway", "unable to send starting round to creator");
        }
        if let Err(e) = self
            .resolved_requests
            .unbounded_send(Request::NewestUnit(self.collection.salt()))
        {
            warn!(target: "StanceBFT-runway", "unable to send resolved request:  {}", e);
        }
        info!(target: "StanceBFT-runway", "Finished initial unit collection with status: {:?}", self.collection.status());
    }

    fn status_report(&self) {
        info!(target: "StanceBFT-runway", "Initial unit collection status report: status - {:?}, collected starting rounds - {}", self.collection.status(), self.collection.collected_starting_rounds);
    }
            ...
            futures::select! {
                response = self.responses_from_network.next() => {
                    let response = match response {
                        ...
                        None => {
                            warn!(target: "StanceBFT-runway", "Response channel closed.");
                            info!(target: "StanceBFT-runway", "Finished initial unit collection with status: {:?}", self.collection.status());
                            return;
                        }
                    };
                    match self.collection.on_newest_response(response) {
                        ...
                        Err(e) => warn!(target: "StanceBFT-runway", "Received wrong newest unit response: {}", e),
                    }
                },
                _ = catch_up_delay => match self.collection.status() {
                    Pending => {
                        delay_passed = true;
                        debug!(target: "StanceBFT-runway", "Catch up delay passed.");
                        self.status_report();
                    },
