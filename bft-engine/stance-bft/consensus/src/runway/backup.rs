...
fn on_shutdown(starting_round_tx: oneshot::Sender<Option<Round>>) {
    if starting_round_tx.send(None).is_err() {
        warn!(target: "StanceBFT-unit-backup", "Could not send `None` starting round.");
    }
}
...
pub async fn run_loading_mechanism<'a, H: Hasher, D: Data, S: Signature, R: Read>(
    ...
    let units = match load_backup(unit_loader, index, session_id) {
        Ok(units) => units,
        Err(e) => {
            error!(target: "StanceBFT-unit-backup", "unable to load unit backup: {}", e);
            on_shutdown(starting_round_tx);
            return;
        }
    };
    let next_round_backup: Round = units.len() as Round;
    info!(target: "StanceBFT-unit-backup", "loaded units from backup. Loaded {:?} units", units.len());

    if let Err(e) = loaded_unit_tx.send(units) {
        error!(target: "StanceBFT-unit-backup", "could not send loaded units: {:?}", e);
        on_shutdown(starting_round_tx);
        return;
    }
    ...
    let next_round_collection = match next_round_collection_rx.await {
        Ok(round) => round,
        Err(e) => {
            error!(target: "StanceBFT-unit-backup", "unable to receive response from unit collections: {}", e);
            on_shutdown(starting_round_tx);
            return;
        }
    };
    info!(target: "StanceBFT-unit-backup", "received next round from unit collection: {:?}", next_round_collection);

    if next_round_backup < next_round_collection {
        error!(target: "StanceBFT-unit-backup", "backup lower than unit collection result. Backup got: {:?}, collection got: {:?}", next_round_backup, next_round_collection);
        on_shutdown(starting_round_tx);
        return;
    };

    if next_round_collection < next_round_backup {
        warn!(target: "StanceBFT-unit-backup", "unit collection result lower than backup. Backup got: {:?}, collection got: {:?}", next_round_backup, next_round_collection);
    }

    if let Err(e) = starting_round_tx.send(Some(next_round_backup)) {
        error!(target: "StanceBFT-unit-backup", "could not send starting round: {:?}", e);
    }
}
